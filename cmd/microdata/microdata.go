...
func main() {
	...
	microdata, using the syntax of package text/template. The default output is
	equivalent to -format '{{. |jsonMarshal }}'. The struct being passed to the
	template is:
...
`)
	...
	flag.Usage = func() {
		...
		flag.PrintDefaults()
		_, _ = fmt.Fprint(os.Stderr, "\nExtract the HTML Microdata and JSON-LD schemas from a HTML document. Format to JSON or using the syntax of package text/template.")
	...
	t, err := template.New("format").Funcs(fnMap).Parse(*format)
	if err != nil {
		fmt.Fprintln(os.Stderr, "error parsing format template:", err)
		os.Exit(1)
	}
